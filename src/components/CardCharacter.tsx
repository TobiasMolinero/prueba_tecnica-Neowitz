...
export default async function CardFilm({ id }: CharacterProps){
    ...
    return(
        <div className="h-[300px] w-[320px] bg-[#1c1c1c67] rounded-lg cursor-pointer group hover:shadow-[0_0_10px_#ffd700] hover:border hover:border-[#ffd90085] transition-shadow">
            <Link href="/" className="h-full w-full flex flex-col items-center justify-center gap-[20px]">
                ...
                <div className='flex flex-col items-center justify-center '>
                    <h3 className="font-[exo2] text-[24px] text-[#00BFFF]  drop-shadow-[0_0_2px_#000] group-hover:scale-[1.1] transition duration-[0.4s]">{data.name}</h3>
                </div>
            </Link>
        </div>
    );
